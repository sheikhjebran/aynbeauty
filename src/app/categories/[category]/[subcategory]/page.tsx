...
export default function SubcategoryPage() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      let allCategoryProducts = data.products || []

      if (subcategorySlug && allCategoryProducts.length > 0) {
        const subcategoryKeywords = subcategorySlug.replace(/-/g, ' ').toLowerCase()
        
        // Find products that match the subcategory
        const subcategoryProducts = allCategoryProducts.filter((product: Product) => 
          product.name.toLowerCase().includes(subcategoryKeywords) ||
          product.description.toLowerCase().includes(subcategoryKeywords)
        )

        // Combine subcategory-specific products with all category products
        // Put subcategory products first, then add remaining category products
        const subcategoryProductIds = new Set(subcategoryProducts.map((p: Product) => p.id))
        const remainingCategoryProducts = allCategoryProducts.filter((product: Product) => 
          !subcategoryProductIds.has(product.id)
        )

        // Final array: subcategory products first, then all other category products
        const combinedProducts = [...subcategoryProducts, ...remainingCategoryProducts]
        setProducts(combinedProducts)
      } else {
        // If no subcategory or no products found, show all category products
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li>
              <div className="flex items-center">
                ...
                <Link href={`/categories/${categorySlug}`} className="ml-1 text-sm font-medium text-gray-700 hover:text-pink-600 md:ml-2">{categoryName}</Link>
              </div>
            </li>
            <li>
              <div className="flex items-center">
                ...
                <span className="ml-1 text-sm font-medium text-gray-500 md:ml-2">{subcategoryName}</span>
              </div>
            </li>
          </ol>
        </nav>
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-lg text-gray-600">
            {subcategoryName} products and more from our {categoryName.toLowerCase()} collection
          </p>
          {products.length > 0 && (
            <p className="mt-1 text-sm text-gray-500">
              {(() => {
                if (subcategorySlug) {
                  const subcategoryKeywords = subcategorySlug.replace(/-/g, ' ').toLowerCase()
                  const subcategoryMatches = products.filter((product: Product) => 
                    product.name.toLowerCase().includes(subcategoryKeywords) ||
                    product.description.toLowerCase().includes(subcategoryKeywords)
                  ).length
                  
                  if (subcategoryMatches > 0) {
                    return `${subcategoryMatches} ${subcategoryName.toLowerCase()} product${subcategoryMatches !== 1 ? 's' : ''} + ${products.length - subcategoryMatches} more ${categoryName.toLowerCase()} products`
                  }
                }
                return `${products.length} product${products.length !== 1 ? 's' : ''} from ${categoryName.toLowerCase()}`
              })()}
            </p>
          )}
        </div>
        ...
        {error ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {products.map((product, index) => {
              // Check if this product matches the subcategory
              const subcategoryKeywords = subcategorySlug?.replace(/-/g, ' ').toLowerCase() || ''
              const isSubcategoryMatch = subcategoryKeywords && (
                product.name.toLowerCase().includes(subcategoryKeywords) ||
                product.description.toLowerCase().includes(subcategoryKeywords)
              )
              ...
              return (
                <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  {isSubcategoryMatch && (
                    <div className="bg-gradient-to-r from-pink-500 to-purple-600 text-white text-xs font-medium px-3 py-1">
                      {subcategoryName} Match
                    </div>
                  )}
